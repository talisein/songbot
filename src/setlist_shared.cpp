...
setlist_base_command::setlist_base_command(context& ctx, std::string_view cmd_name, std::string_view cmd_desc) noexcept :
    iface_command(ctx, cmd_name, cmd_desc)
{
    success_counter        = &ctx.slashcommand_counter->Add({{"command", std::string(cmd_name)}, {"result", "success"}, {"visibility", "ephemeral"}});
    failure_counter        = &ctx.slashcommand_counter->Add({{"command", std::string(cmd_name)}, {"result", "failure"}, {"visibility", "ephemeral"}});
    reveal_success_counter = &ctx.slashcommand_counter->Add({{"command", std::string(cmd_name)}, {"result", "success"}, {"visibility", "public"}});
    reveal_failure_counter = &ctx.slashcommand_counter->Add({{"command", std::string(cmd_name)}, {"result", "failure"}, {"visibility", "public"}});
    ac_success_counter     = &ctx.autocompletion_counter->Add({{"event", std::string(cmd_name)}, {"result", "success"}});
    ac_no_match_counter    = &ctx.autocompletion_counter->Add({{"event", std::string(cmd_name)}, {"result", "no-match"}});
    ac_failure_counter     = &ctx.autocompletion_counter->Add({{"event", std::string(cmd_name)}, {"result", "failure"}});
}

dpp::slashcommand
setlist_base_command::get_command()
{
    ...
    cmd.add_option(std::move(event_opt));
    return cmd;
}

dpp::task<std::expected<void, std::error_code>>
setlist_base_command::reply_multimessage(const dpp::slashcommand_t& event,
                                         const Concert& concert)
{
    ...
    auto body_lines = get_body_lines(concert);
    ...
    { // Button clicked
        ...
        if (auto res = co_await reply_and_followup(click_event, reveal_msgs, ctx); !res) {
            reveal_failure_counter->Increment();
            co_return res;
        }
        ...
        co_return co_await util::reply_handler_new(event.co_edit_original_response(success_msg), ctx, reveal_success_counter, reveal_failure_counter);
        ...
        co_return co_await util::reply_handler_new(event.co_edit_original_response(*std::ranges::begin(expired_msgs)), ctx, reveal_success_counter, reveal_failure_counter);
    }
...
dpp::task<std::expected<void, std::error_code>>
setlist_base_command::on_slashcommand(const dpp::slashcommand_t event)
{
    try {
        ...
        if (!concert) {
            ...
            auto res = co_await util::reply_handler_new(event.co_reply(m), ctx, success_counter, failure_counter);
        ...
        m.set_flags(dpp::message_flags::m_ephemeral);
        event.reply(m);
        ctx->log_error("/{}: System Error {}", cmd_name, e.what());
        failure_counter->Increment();
        ...
        m.set_flags(dpp::message_flags::m_ephemeral);
        event.reply(m);
        ctx->log_error("/{}: std::get() {}", cmd_name, e.what());
        failure_counter->Increment();
...
setlist_base_command::on_autocomplete_impl(const dpp::autocomplete_t& event)
{
    for (const auto& opt : std::views::all(event.options) | std::views::filter(&dpp::command_option::focused)) {
        try {
            ...
            ctx->log_error("/{}: autocomplete error: {}", cmd_name, e.what());
...
std::expected<dpp::interaction_response, std::error_code>
setlist_base_command::on_autocomplete(const dpp::autocomplete_t& event)
{
    auto res = on_autocomplete_impl(event);
    if (res) {
        ac_success_counter->Increment();
    } else {
        ...
        if (is_no_match) {
            ac_no_match_counter->Increment();
        } else {
            ac_failure_counter->Increment();
        }
    }
