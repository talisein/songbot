...
namespace {
    ...
    get_setlist_lines(const Concert& concert)
    {
        ...
        if (std::ranges::distance(setlist) == 0 && concert.date > util::get_build_date()) {
...
setlist_command::setlist_command(context& ctx) noexcept :
    setlist_base_command(ctx, "setlist", "Full Concert Setlist")
{
}

std::vector<std::string>
setlist_command::get_body_lines(const Concert& concert)
{
    return get_setlist_lines(concert);
}
