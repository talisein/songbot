...
namespace {
    ...
    get_setlist_chunks(const Concert &concert, size_t header_size)
    {
        ...
        for (const auto& line : get_setlist_lines(concert))
        {
                                        ...
                                        const Concert& concert)
{
    ...
    { // Button clicked
        ...
        co_return co_await util::reply_handler_new(event.co_edit_original_response(*std::ranges::begin(expired_gui_msgs)), ctx, setlist_reveal_success_counter, setlist_reveal_failure_counter);
    }
