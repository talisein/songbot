...
int main()
{
    ...
    "song_is_game_track: true for game song"_test = [] {
        auto song = lookup_song("Electric Angel");
        expect(song.has_value());
        expect(song_is_game_track(*song));
    };

    "song_is_game_track: false for song not in any game"_test = [] {
        auto song = lookup_song("Over Flow(er)");
        expect(song.has_value());
        expect(not song_is_game_track(*song));
    };
